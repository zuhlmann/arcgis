...
def get_overlap(ds1, ds2):
    ...
    # y_offset1 = [math.ceil(r2[1] - intersection[1]), math.floor(r2[1] - intersection[3])]
...
reservoir_storage_copco = round(np.mean(arr_storage_copco))
...
reservoir_min_change_copco = round(np.min(arr_masked_copco_diff), 1)
reservoir_max_change_copco = round(np.max(arr_masked_copco_diff), 1)
acre_feet_copco = np.mean(arr_storage_copco) * reservoir_acre_copco
pct_storage_change_copco = (-1) *  np.mean(arr_masked_copco_diff / arr_storage_copco) * 100
...
reservoir_storage_IG = round(np.mean(arr_storage_IG))
reservoir_acre_IG = round(np.sum(mask_IG) * acres_convert, 1)
reservoir_mean_change_IG = round(np.mean(arr_masked_IG_diff), 1)
reservoir_min_change_IG = round(np.min(arr_masked_IG_diff), 1)
reservoir_max_change_IG = round(np.max(arr_masked_IG_diff), 1)
acre_feet_IG = np.mean(arr_storage_IG) * reservoir_acre_IG
pct_storage_change_IG = (-1) *  np.mean(arr_masked_IG_diff / arr_storage_IG) * 100
...
df = pd.DataFrame(np.column_stack([res_mean, res_min, res_max, res_acre, res_storage, acre_feet,
                ...
                columns = ['mean sediment change (feet)', 'min sediment change (feet)',
                        'max sediment change (feet)', 'reservoir size (acres)', 'res storage',
...
# # fp_out = os.path.join(utilities.get_path(22), 'bathymetry_project/{}_Sediment_Change_2018_2010.png'.format(reservoir))
